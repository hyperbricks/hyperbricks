...
The Goja version is around 20 times slower than the very small native Go baseline, but its total measured time is about `0.015 ms` per render. This [benchmark](../pkg/component/goja_render_benchmark_test.go) measures one component in isolation; it does not represent complete HTTP latency or maximum server capacity.
