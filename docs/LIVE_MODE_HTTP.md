...
`live.cache` controls how long rendered output may be stored and reused in live mode.
...
Set `nocache: true` on the routed component when the response depends on the current request.
