#!/usr/bin/env python3
"""Exercise Project Desk through the real CLI and HTTP server in temporary projects.
...
MODULE = ROOT / "modules/hyperbricks-basics"
NAME = "hyperbricks-basics"
...
def verify_core(binary, workspace):
    ...
    with server([binary, "start", "-m", NAME, "-p", port, "--non-interactive"], project,
        ...
        pages = {"/": "Project Desk", "/projects": "Atlas", "/projects/atlas": "Atlas",
                 "/projects/beacon": "Beacon", "/estimate?quantity=3": "€59.85", "/handbook": "handbook"}
        assets = set()
        ...
        # Exercise the published extension recipe itself, not a separate example.
        lesson = (module / "README.md").read_text().split("## Add a page and its fragment", 1)[1]
        snippets = re.findall(r"```yaml\n(.*?)```", lesson.split("## How the pieces work together", 1)[0], re.S)
        check(len(snippets) == 2, "The README extension exercise changed; update this check")
        (module / "hyperbricks/about.hyperbricks.yaml").write_text(snippets[0])
        navigation = module / "hyperbricks/partials/navigation.hyperbricks.yaml"
        navigation.write_text(navigation.read_text() + snippets[1])
        for _ in range(60):
            time.sleep(0.2)
        ...
        check("About this workspace" in page and "About this workspace" in fragment,
              "Documented page and fragment do not share their view")
        check('href="/about"' in page and page.count('id="main-content"') == 1,
              "Documented navigation or shared shell is missing")
        print("PASS README add-page, fragment, and navigation exercise", flush=True)
...
def verify_packaging(binary, workspace):
    ...
    static_project = workspace / "handbook"
    module = staging.stage(MODULE, static_project, handbook=True)
    run([binary, "static", "-m", NAME, "--force", "--zip", "--non-interactive"], static_project,
        workspace / "static.log")
    rendered = module / "rendered"
    check((rendered / "index.html").is_file(), "Static handbook missing")
    document = (rendered / "index.html").read_text()
    check("handbook" in document.lower() and "<form" not in document and "hx-get" not in document,
          "Static handbook depends on runtime interaction")
    ...
    print("PASS public static handbook and export archive", flush=True)
...
def verify_api(binary, workspace):
    ...
    base = f"http://127.0.0.1:{port}"
    env = {**os.environ, "HYPERBRICKS_BASICS_API_URL": api}
    with server([fixture, "-port", api_port], project, workspace / "fixture.log", api + "/health"):
        with server([binary, "start", "-m", NAME, "--config", "package.advanced.hyperbricks.yaml",
...
def verify_plugin(binary, workspace):
    ...
    run([binary, "plugin", "build", "project-desk@1.0.0", "--module", NAME], project,
        workspace / "plugin-build.log", env)
    run(["go", "test", "./..."], module / "plugins/project-desk/1.0.0", workspace / "plugin-test.log")
    port = free_port()
    base = f"http://127.0.0.1:{port}"
    with server([binary, "start", "-m", NAME, "--config", "lessons/plugin/package.hyperbricks.yaml",
...
def main():
    ...
    parser.add_argument("--with-plugin", action="store_true", help="also build and check the optional native Go plugin")
    args = parser.parse_args()
    workspace = Path(tempfile.mkdtemp(prefix="hb-basics-"))
    ...
    print("All Project Desk CLI/HTTP checks passed.", flush=True)
