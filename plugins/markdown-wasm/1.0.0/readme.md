...
This is a Go-authored WebAssembly plugin for the HyperBricks WASM plugin
runtime. The CLI builds it with `GOOS=wasip1 GOARCH=wasm`, and the runtime
provides WASI imports without mounting a filesystem.
...
Go/WASI modules also export `_initialize`; HyperBricks calls it before invoking
`alloc` and `render`.
