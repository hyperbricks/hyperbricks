...
func TestAPIRenderExplicitAuthentication(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cfg := APIConfig{ApiRenderConfig: settings}
			output, errs := (&APIRenderer{}).Render(cfg, ctx)
			if tt.wantErr {
				if len(errs) == 0 {
			...
			if tt.wantJWT {
				assertAPIRenderSignedAuthorization(t, got.Get("Authorization"), tt.jwtSecret, settings.JwtClaims)
				return
			}
...
func assertAPIRenderSignedAuthorization(t *testing.T, authorization, secret string, expectedClaims map[string]string) {
	t.Helper()
