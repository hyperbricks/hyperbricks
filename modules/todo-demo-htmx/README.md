...
A small todo app served by HyperBricks, using **HTMX 4.0.0** for HTML fragment updates and **localStorage** for persistence.
...
mkdir -p modules/todo-demo-htmx/rendered
hyperbricks start -m todo-demo-htmx
```

Open [localhost:8121](http://localhost:8121/). Native esbuild builds the JavaScript and CSS automatically. The browser library is included locally; no npm install, CDN connection, plugin build or extra server is needed.
...
4. Open [How it works](http://localhost:8121/how-it-works) for the component overview.
...
| The HTMX integration | [integration.js](resources/js/integration.js) |

`htmx.ajax()` requests the fragment and swaps the outer HTML of `#todo-panel`.
...
- Storage keys: `todo-demo-htmx:tasks:v1` and `todo-demo-htmx:scratchpad:v1`. Each demo has a different key. Storage is scoped to the browser profile and origin, not to a logged-in account; tabs on the same origin share it.
