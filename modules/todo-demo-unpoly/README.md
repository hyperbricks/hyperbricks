...
A small todo app served by HyperBricks, using **Unpoly 3.14.3** for HTML fragment updates and **localStorage** for persistence.
...
mkdir -p modules/todo-demo-unpoly/rendered
...
| The Unpoly integration | [integration.js](resources/js/integration.js) |

`up.render()` requests the fragment and replaces `#todo-panel`, with history changes, client caching and full-page fallback disabled.
...
- Storage keys: `todo-demo-unpoly:tasks:v1` and `todo-demo-unpoly:scratchpad:v1`. Each demo has a different key. Storage is scoped to the browser profile and origin, not to a logged-in account; tabs on the same origin share it.
