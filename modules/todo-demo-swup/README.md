...
Open [localhost:8124](http://localhost:8124/). Native esbuild builds the JavaScript and CSS automatically. The browserlibrary is included locally; no npm install, CDN connection, plugin build or extra server is needed.
...
4. Open [How it works](http://localhost:8124/about) for the component overview.
...
These are independent modules with the same UI and application logic. Differences are the library, its integration file, and their panel integration. They prove this todo/fragment workflow, not every feature of each library.
